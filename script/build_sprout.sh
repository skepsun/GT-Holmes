...
python python/lib/file2std.py data/BurglaryCFS.xlsx     table 1 | \
awk -F '\t' 'BEGIN {OFS = FS} {t = $NF; $NF = $2; $2 = t; print;}' > \
	${workspace_dir}/BurglaryCFS.stream

# - Data streaming 2
#   Rearrange cols: 1. incident no; 2. tag; ...
python python/lib/file2std.py data/BurglaryOffCore.xlsx table 2 | \
awk -F '\t' 'BEGIN {OFS = FS}  {t = $NF; $NF = $2; $2 = t; print;} ' > \
	${workspace_dir}/BurglaryOffCore.stream

# - Data streaming 3
#   Rearrange cols: 1. incident no; 2. tag; ...
python python/lib/file2std.py data/BurglaryRemarks.xlsx table 3 | \
